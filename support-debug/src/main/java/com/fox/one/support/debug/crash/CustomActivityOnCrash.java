...
public final class CustomActivityOnCrash {
    ...
    private static F1CrashConfig config = new F1CrashConfig();
                        ...
                        public void uncaughtException(Thread thread, final Throwable throwable) {
                            if (config.isEnabled()) {
                                ...
                                if (hasCrashedInTheLastSeconds(application)) {
                                    ...
                                    } else if (config.getBackgroundMode() == F1CrashConfig.BACKGROUND_MODE_SHOW_CUSTOM || !isInBackground) {
                                    ...
                                    } else if (config.getBackgroundMode() == F1CrashConfig.BACKGROUND_MODE_CRASH) {
                                        if (oldHandler != null) {
                                            oldHandler.uncaughtException(thread, throwable);
                                            return;
                                        }
    ...
    @NonNull
    public static F1CrashConfig getConfigFromIntent(@NonNull Intent intent) {
        return (F1CrashConfig) intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_CONFIG);
    }
    ...
    public static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull F1CrashConfig config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull F1CrashConfig config) {
        Intent intent = new Intent(activity, config.getRestartActivityClass());
        restartApplicationWithIntent(activity, intent, config);
    }

      
    public static void closeApplication(@NonNull Activity activity, @NonNull F1CrashConfig config) {
        if (config.getEventListener() != null) {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static F1CrashConfig getConfig() {
        return config;
    }

      
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull F1CrashConfig config) {
