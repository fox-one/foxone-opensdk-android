...
interface IOtcAPI {
     ...
     * 获取用户的所有资产（资产列表）
     */
    @GET("/member/otc/assets")
    fun getAssets(): FoxCall<List<AssetInfo>>

    /**
     * 获取支持转入的资产列表
     */
    @GET("/member/otc/assets?chain=1")
    fun getMainChainAssets(): FoxCall<List<AssetInfo>>

    /**
     * 获取单个资产详情
     */
    @GET("/member/otc/asset/{assetId}")
    ...
    @GET("/member/otc/snapshot/{snapshotId}")
